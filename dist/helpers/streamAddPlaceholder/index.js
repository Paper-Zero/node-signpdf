...
const streamAddPlaceholder = async ({
  ...
  const tempPath = output === pdfPath ? `${pdfPath}.tmp` : output; // Primeiro, analisar a estrutura do PDF para encontrar onde inserir a assinatura

  const pdfInfo = await _analyzePdfStructure(pdfPath); // Gerar os objetos necessários para a assinatura
  ...
  await _writePdfWithPlaceholder(pdfPath, tempPath, pdfInfo, signatureObjects); // Se usamos arquivo temporário, renomear

  if (tempPath !== output) {
    await _fs.promises.rename(tempPath, output);
  }
...
async function _analyzePdfStructure(pdfPath) {
  return new Promise((resolve, reject) => {
    ...
    stream.on('data', chunk => {
      buffer = Buffer.concat([buffer, chunk]); // Procurar por xref
    ...
    stream.on('end', async () => {
      if (!rootRef) {
        reject(new _SignPdfError.default('Could not find PDF root object reference.', _SignPdfError.default.TYPE_PARSE));
        return;
      }
...
async function _writePdfWithPlaceholder(inputPath, outputPath, pdfInfo, signatureObjects) {
  ...
  return new Promise((resolve, reject) => {
    readStream.on('data', chunk => {
      buffer = Buffer.concat([buffer, chunk]); // Procurar pelo início da tabela xref
...
function _modifyXref(xrefContent, signatureObjects) {
  ...
  content = content.replace(/xref\s+\d+\s+\d+/, `xref\n${startNum} ${newCount}`); // Adicionar entradas para os novos objetos (calcular offsets aproximados)
  ...
  if (trailerIndex !== -1) {
    content = content.slice(0, trailerIndex) + newEntries + content.slice(trailerIndex); // Modificar trailer para incluir AcroForm

    content = content.replace(/trailer\s*<<([^>]*)>>/, `trailer\n<<$1/AcroForm ${signatureObjects.acroFormRef}>>`);
  }
